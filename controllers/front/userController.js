...
async function profileUpdate(req, res) {
  ...
  const validationResponse = validator.validate(
    ...
    },
    {
      ...
      officeAddress: { type: 'string', empty: false, max: '120' },
      service1: { type: 'string', empty: false, max: '20' },
      service2: { type: 'string', empty: false, max: '20' },
      service3: { type: 'string', empty: false, max: '20' },
    }
  );
  ...
  const updateUser = {
    ...
    companyLogo: imgname? imgname:checkUser.companyLogo,
...
async function authGoogleCallbackSuccess(req, res) {
  ...
  if (checkEmail) {
    if (checkEmail.authprovider == null) {
      ...
      req.flash(
        'error',
        'Allready Registered this email with facebook login. '
      );

      return res.redirect('/home');
    } else if (
      checkEmail.authprovider == 'google' &&
      checkEmail.authid == req.user.id
    ) {
      // facebook login//
...
async function authFacebookCallbackSuccess(req, res) {
  ...
  if (checkEmail) {
    if (checkEmail.authprovider == null) {
      ...
      checkEmail.authprovider == 'facebook' &&
      checkEmail.authid == req.user.id
    ) {
      // facebook login//
