...
async function festivalThemeList(req, res) {
  ...
  if (loginAuthCheck) {
    ...
    //console.log(userinfo.holidayThemes);
  ...
  // }
  //console.log(allFestivalThemeToArray);
...
async function userFavHolidayTheme(req, res) {
  ...
  const allFavHolidayData = await Models.Subcategory.findAll({
    where: {
      ...
      [Op.and]: [{ categoryId: 3 }, { festivalSubCategoryId: { [Op.in]: allHolidayThemeToArray } }]
    },
    include: [{
      model: Models.Category,
      attributes: ["name"],
...
async function getNewFestivalAjaxSort(req, res) {
  ...
  if (loginAuthCheck) {
    ...
    return res.json({ msg: 'updated', subcategorys: allfestivalResultSort });
  } else {
    return res.json({ msg: 'updated', subcategorys: allfestivalResultSort });
  }
