      ...
      if (today > TODAY_START && today < TODAY_END) {
        ...
        cron.schedule('* * * * *', () => {
          ...
          contacts.forEach((contact, i) => {
            //var url = `http://localhost:9128/temp/review/download/${userId}/${userInfo.defaultBirthdayTheme}/${contact.id}/false/true`;
    ...
    userInfo.forEach(async (user,index) => {
      ...
      if (contacts.length > 0) {
        if (today > TODAY_START && today < TODAY_END) {
          ...
          contacts.forEach((contact, i) => {
            //var url = `http://localhost:9128/temp/review/download/${userId}/${userInfo.defaultBirthdayTheme}/${contact.id}/false/true`;
            //var url = `http://localhost:9128/temp/review/download/${user.id}/${randomDefaultBirthdayTheme}/${contact.id}/false/true`;
            var url = `${BASEPATH}/temp/review/download/${user.id}/${randomDefaultBirthdayTheme}/${contact.id}/false/true`;
            console.log(url);
            //htmltocanvas code//
            console.log('cron working');
      ...
      if (anniversarys.length > 0) {
        ...
        if (today > TODAY_START && today < TODAY_END) {
          ...
          cron.schedule('* * * * *', () => {
            ...
            anniversarys.forEach((anniversary, i) => {
              //var url = `http://localhost:9128/temp/review/download/${userId}/${userInfo.defaultBirthdayTheme}/${contact.id}/false/true`;
              var url = `${BASEPATH}/temp/review/download/${user.id}/${randomDefaultBirthdayTheme}/${anniversary.id}/false/true`;
              console.log(url);
              //htmltocanvas code//
              console.log('cron working');
              const transporter = nodemailer.createTransport({
